...
test('Installed real Ferrari preserves source surfaces and materials across 131 prepared assemblies', async () => {
  const ferrariSource=await inspectSource(fileURLToPath(new URL('../public/assets/ferrari.glb',import.meta.url)),'position');
  ...
  assert.equal(ferrariManifest.parts.length,131);
  assert.deepEqual(ferrariManifest.parts.map(part=>part.id),ferrariMapping.parts.map(part=>part.id));
  assert.equal(ferrariManifest.parts.filter(part=>part.family).length,6);
  ...
  const outputSourceIds=ferrariOutput.doc.getRoot().listNodes().map(node=>node.getExtras().sourcePiece).filter(Boolean);
  assert.equal(new Set(outputSourceIds).size,ferrariSource.pieces.length);
  assert.equal(outputSourceIds.length,ferrariSource.pieces.length);
