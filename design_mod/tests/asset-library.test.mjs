...
import {vehicle, vehicles} from '../catalog.mjs';
import {repairCatalog} from '../repair-catalog.mjs';

test('Design and repair retain stable vehicle identities and their separate prepared-asset contracts', async () => {
  ...
  assert.equal(vehicle.id, ferrariVehicle.id);
  assert.deepEqual(vehicles.map(item=>item.id), ['ferrari-458-demo','toyota-corolla-2020','porsche-911-carrera-4s']);
  ...
  assert.equal(ferrari.prepared.parts.length,131);
  assert.deepEqual(ferrari.prepared.parts.filter(part=>part.family).map(part=>part.id).sort(),['front-bumper','hood','left-front-fender','left-mirror','right-front-fender','right-mirror']);
  assert.equal(ferrari.repairNotice,undefined);
